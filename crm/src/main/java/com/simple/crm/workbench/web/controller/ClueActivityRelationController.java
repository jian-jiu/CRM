...
public class ClueActivityRelationController {
     ...
     *
     * @param clueId     线索id
     * @param activityId 市场id
     * @return 结果集
     */
    @RequestMapping("addClueActivityRelation")
    public Object addClueActivityRelation(String clueId, String[] activityId) {
        int i = clueActivityRelationService.addClueActivityRelation(clueId, activityId);
        List<Activity> activityList = activityService.findActivityByClueId(clueId);
        ReturnObject returnObject = new ReturnObject();
        returnObject.setCode(Contents.RETURN_OBJECT_CODE_SUCCESS);
        returnObject.setMessage(String.valueOf(i));
